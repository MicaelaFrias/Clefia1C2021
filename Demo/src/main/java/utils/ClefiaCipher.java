...
public abstract class ClefiaCipher {
    // S0 (8-bit S-box based on four 4-bit S-boxes)
    private static final int[] clefia_s0 = {
    ...
    private static final int[] clefia_s1 = {
    ...
    public static Byte ClefiaMul8(Byte x) {
        return ClefiaMul2(ClefiaMul4(x));
    }
    ...
    public static void ClefiaF0XorTest(List<Byte> dst, int offsetDst,
        ...
        FunctionHelper.addSpaceToList(x, 4, 4);
        // F0
        // Key addition
        ...
        // Add space to list because size is 0, length = 4 because adding 4 in next function
        FunctionHelper.addSpaceToList(dst, 4, 16);
        // Xoring after F0
        FunctionHelper.copyList(dst, offsetDst, src, offsetSrc, 4);
    ...
    public static void ClefiaGfn4Test(List<Byte> y, List<Byte> x, List<Byte> rk, int offsetRk, int r) {
        ...
        FunctionHelper.addSpaceToList(fin, 16, 16);
        FunctionHelper.copyList(fin, 0, x, 0, 16);
        ...
        while (r-- > 0) {
            ...
            if (r != 0) { // swapping for encryption
                FunctionHelper.copyList(fin, 0, fout, 4, 12);
                FunctionHelper.copyList(fin, 12, fout, 0, 4);
            }
        }

        FunctionHelper.copyList(y, 0, fout, 0, 16);
    }
    ...
    public static void ClefiaGfn4InvTest(List<Byte> y, List<Byte> x, List<Byte> rk, int offsetRk, int r) {
        ...
        FunctionHelper.addSpaceToList(fout, 16, 16);
        FunctionHelper.addSpaceToList(fin, 16, 16);
        ...
        while (r-- > 0) {
            ...
            if (r != 0) { // swapping for encryption
                FunctionHelper.copyList(fin, 0, fout, 12, 4);
                FunctionHelper.copyList(fin, 4, fout, 0, 12);
            }
        }
    ...
    public static void ClefiaGfn8Test(List<Byte> y, List<Byte> x, List<Byte> rk, int offsetRk, int r) {
        ...
        FunctionHelper.addSpaceToList(fout, 32, 32);
        FunctionHelper.addSpaceToList(fin, 32, 32);

        FunctionHelper.copyList(fin, 0, x, 0, 32);
        ...
        while (r-- > 0) {
            ...
            if (r != 0) { // swapping for encryption
                FunctionHelper.copyList(fin, 0, fout, 4, 28);
                FunctionHelper.copyList(fin, 28, fout, 0, 4);
            }
        }

        FunctionHelper.copyList(y, 0, fout, 0, 32);
    }

    public static int ClefiaKeySet(List<Byte> rk, List<Byte> skey, int key_bitlen) {
        if (128 == key_bitlen) {
    ...
    public static void ClefiaDoubleSwapTest(List<Byte> lk, int offsetLk) {
        ...
        t.add((byte) ((getPositiveFromByte(lk.get(offsetLk)) << 7) |
        ...
        t.add((byte) ((getPositiveFromByte(lk.get(8 + offsetLk)) >> 7) |
                (getPositiveFromByte(lk.get(offsetLk)) & 0xfe)));
        ...
        FunctionHelper.copyList(lk, offsetLk, t, 0, 16);
    }
    ...
    public static void ClefiaKeySet128(List<Byte> rk, List<Byte> skey) {
        ...
        FunctionHelper.addSpaceToList(lk, 16, 16);
        ...
        FunctionHelper.copyList(rk, offsetRk, skey, offsetSkey, 8); // initial whitening key (WK0, WK1)
        offsetRk += 8;
        ...
        FunctionHelper.copyList(rk, offsetRk, skey, 8, 8); // final whitening key (WK2, WK3)
    }
    ...
    public static void ClefiaKeySet192(List<Byte> rk, List<Byte> skey) {
        ...
        FunctionHelper.addSpaceToList(lk, 32, 32);
        FunctionHelper.addSpaceToList(skey256, 32, 32);

        FunctionHelper.copyList(skey256, 0, skey, 0, 24);
    ...
    public static void ClefiaEncryptTest(List<Byte> ct, List<Byte> pt, List<Byte> rk, int r) {
        ...
        FunctionHelper.addSpaceToList(rout, 16, 16);
        FunctionHelper.addSpaceToList(rin, 16, 16);

        FunctionHelper.copyList(rin, 0, pt, 0, 16);
        ...
        FunctionHelper.copyList(ct, 0, rout, 0, 16);
    ...
    public static void ClefiaDecryptTest(List<Byte> pt, List<Byte> ct, List<Byte> rk, int r) {
        ...
        FunctionHelper.copyList(rin, 0, ct, 0, 16);
        ...
        FunctionHelper.copyList(pt, 0, rout, 0, 16);

        // Esto estaba como offsetRK - 8
        ByteXorTest(pt, 4, pt, 4, rk, 0, 4); // final key whitening
        ByteXorTest(pt, 12, pt, 12, rk, offsetRk - 4, 4);
    }

    public static void chargeSkey(List<Byte> skey) {
        skey.add((byte) 0xff);
        skey.add((byte) 0xee);
        skey.add((byte) 0xdd);
        skey.add((byte) 0xcc);
